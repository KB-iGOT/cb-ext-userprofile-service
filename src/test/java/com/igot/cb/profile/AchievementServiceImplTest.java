...
class AchievementServiceImplTest {
    ...
    // ==================== VOLUNTEER USER ACHIEVEMENTS TESTS ====================

    @Test
    void testGetVolunteerUserAchievements_success() {
        Map<String, Object> request = new HashMap<>();
        request.put(Constants.REQUEST, Map.of(Constants.USER_ID_RQST, "user-123"));

        when(accessTokenValidator.fetchUserIdFromAccessToken("token")).thenReturn("volunteer-user");

        // Create a spy of the achievement service to mock the getUserAchievements method
        AchievementServiceImpl spyService = spy(achievementService);
        ApiResponse mockResponse = new ApiResponse();
        mockResponse.setResponseCode(HttpStatus.OK);
        doReturn(mockResponse).when(spyService).getUserAchievements(anyString(), anyString());

        ApiResponse response = spyService.getVolunteerUserAchievements("token", request);

        assertEquals(HttpStatus.OK, response.getResponseCode());
        verify(spyService).getVolunteerUserAchievements("token", request);
    }

    @Test
    void testGetVolunteerUserAchievements_invalidToken_returnsUnauthorized() {
        Map<String, Object> request = new HashMap<>();
        request.put(Constants.REQUEST, Map.of(Constants.USER_ID_RQST, "user-123"));

        when(accessTokenValidator.fetchUserIdFromAccessToken("bad-token")).thenReturn(null);

        ApiResponse response = achievementService.getVolunteerUserAchievements("bad-token", request);

        // The method should delegate to getUserAchievements which will return based on token validation
        assertNotNull(response);
    }

    @Test
    void testGetVolunteerUserAchievements_withEmptyRequest() {
        Map<String, Object> request = new HashMap<>();
        Map<String, Object> requestData = new HashMap<>();
        request.put(Constants.REQUEST, requestData);

        when(accessTokenValidator.fetchUserIdFromAccessToken("token")).thenReturn("user-123");

        ApiResponse response = achievementService.getVolunteerUserAchievements("token", request);

        assertNotNull(response);
    }

    @Test
    void testGetVolunteerUserAchievements_withMultipleUsers() {
        Map<String, Object> request = new HashMap<>();
    ...
    @Test
    void testGetVolunteerUserAchievements_extractsUserIdFromRequest() {
        String expectedUserId = "volunteer-123";
        Map<String, Object> request = new HashMap<>();
        Map<String, Object> requestData = new HashMap<>();
        requestData.put(Constants.USER_ID_RQST, expectedUserId);
        request.put(Constants.REQUEST, requestData);

        when(accessTokenValidator.fetchUserIdFromAccessToken("token")).thenReturn("admin-user");

        AchievementServiceImpl spyService = spy(achievementService);
        ApiResponse mockResponse = new ApiResponse();
        mockResponse.setResponseCode(HttpStatus.OK);
        doReturn(mockResponse).when(spyService).getUserAchievements(anyString(), eq(expectedUserId));

        ApiResponse response = spyService.getVolunteerUserAchievements("token", request);

        // Verify that the user ID was correctly extracted and passed
        verify(spyService).getVolunteerUserAchievements("token", request);
    }
